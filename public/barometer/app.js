...
const state = { latitude: null, longitude: null, loading: false, deferredInstallPrompt: null, pressureController: null };
...
function setLoading(on){ state.loading=on; if(elements.refreshConditions) elements.refreshConditions.disabled=on; if(on){ elements.weatherCondition.textContent="Updating"; elements.locationStatus.textContent="Getting current conditions"; } }
function useLocation(latitude, longitude, label){ state.latitude=latitude; state.longitude=longitude; elements.locationName.textContent=label; elements.catchLocation.value=`${latitude.toFixed(4)}, ${longitude.toFixed(4)}`; loadConditions(); }
function getLocation(){ setLoading(true); if(!navigator.geolocation){ useLocation(FALLBACK_LOCATION.latitude,FALLBACK_LOCATION.longitude,FALLBACK_LOCATION.label); return; } navigator.geolocation.getCurrentPosition(p=>{ elements.locationStatus.textContent="Location connected"; elements.locationStatusDot.classList.add("locked"); useLocation(p.coords.latitude,p.coords.longitude,"Current location"); },()=>{ elements.locationStatus.textContent="Location unavailable — showing Carlyle Lake"; elements.locationStatusDot.classList.add("error"); useLocation(FALLBACK_LOCATION.latitude,FALLBACK_LOCATION.longitude,FALLBACK_LOCATION.label); },{timeout:10000,maximumAge:300000}); }

async function loadConditions(){ if(state.loading && state.latitude===null) return; if(state.latitude===null) return getLocation(); setLoading(true); await Promise.allSettled([loadPressure(),loadWeather()]); elements.lastUpdated.textContent=`Updated ${new Date().toLocaleTimeString([], {hour:"numeric",minute:"2-digit"})}`; setLoading(false); }

async function loadPressure(){
  if(state.pressureController) state.pressureController.abort(); state.pressureController=new AbortController();
  try{
    ...
    const data=await response.json(); if(!response.ok || !data.latest) throw new Error(data.error || "Pressure data unavailable");
    const pressure=Number(data.latest.pressureInHg); elements.pressureValue.textContent=Number.isFinite(pressure)?pressure.toFixed(2):"--.--";
    elements.locationName.textContent=data.station?.name || elements.locationName.textContent; elements.pressureTrend.textContent=data.guidance || "Current pressure loaded";
    const meta=trendMeta[data.trend] || trendMeta.steady; elements.biteStatus.textContent=meta.badge; elements.biteStatus.className=`status-pill ${meta.cls}`.trim();
    elements.pressureNeedle.style.transform=`translateX(-50%) rotate(${meta.angle}deg)`; if(Array.isArray(data.series)&&data.series.length>1) drawTrendLine(data.series);
  }catch(error){ if(error.name!=="AbortError"){ elements.pressureTrend.textContent="Pressure data unavailable right now"; elements.biteStatus.textContent="N/A"; } }
}
...
async function loadWeather(){ try{ const response=await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${state.latitude}&longitude=${state.longitude}&current=temperature_2m,weather_code&temperature_unit=fahrenheit`); const data=await response.json(); if(!response.ok||!data.current)throw new Error(); elements.airTemperature.textContent=`${Math.round(data.current.temperature_2m)}°`; elements.weatherCondition.textContent=weatherDescriptions[data.current.weather_code]||"Current conditions"; }catch{ elements.airTemperature.textContent="--°"; elements.weatherCondition.textContent="Weather unavailable"; } }
...
function registerServiceWorker(){ if("serviceWorker" in navigator) navigator.serviceWorker.register("/barometer/sw.js").catch(console.warn); }
