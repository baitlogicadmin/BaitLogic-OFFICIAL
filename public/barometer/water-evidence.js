...
const waterStyle=document.createElement("style");waterStyle.textContent=`.water-evidence-card{padding:22px}.water-evidence-grid{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-top:14px}.water-evidence-grid article{padding:13px;border:1px solid var(--line);border-radius:15px;background:linear-gradient(145deg,rgba(8,39,50,.92),rgba(4,24,33,.96));box-shadow:inset 0 1px 0 rgba(255,255,255,.025)}.water-evidence-grid span{display:block;color:var(--muted);font-size:.66rem;font-weight:900;text-transform:uppercase;letter-spacing:.08em}.water-evidence-grid strong{display:block;margin-top:6px;color:var(--white);font-size:.94rem;line-height:1.3}.water-source{display:block;margin-top:12px;color:var(--muted);font-size:.68rem;line-height:1.4}@media(max-width:620px){.water-evidence-grid{grid-template-columns:1fr 1fr}}`;document.head.appendChild(waterStyle);
async function loadWaterEvidence(){
  ...
  },()=>{Wset("#waterEvidenceState","NO GPS");Wset("#waterEvidenceNote","Allow location to match nearby public water measurements. No substitute location will be used.");},{enableHighAccuracy:false,timeout:10000,maximumAge:300000});
}
