...
export type DashboardSnapshot = {
  ...
  weather: {
    temperatureF: number; apparentTemperatureF?: number; humidity?: number; code: number;
    pressureInHg: number; pressureDelta3h: number; pressureDelta6h?: number;
    windMph: number; windDirection: number; gustMph?: number; cloudCover?: number;
    sunrise?: string | null; sunset?: string | null;
  };
};
export type WaterSnapshot = {
  source: string;
  stations: Array<{ site:string; name:string; flow:number|null; gage:number|null; temp:number|null }>;
  timestamp: string;
};
...
export const WEATHER_LABELS: Record<number,string>={
  ...
  81:"Rain showers",82:"Heavy showers",95:"Thunderstorms",96:"Thunderstorms / hail",99:"Severe thunderstorms / hail"
};
...
export function formatClock(value?:string|null){
  ...
  if(localClock){const hour24=Number(localClock[1]);const minute=localClock[2];if(Number.isFinite(hour24))return `${hour24%12||12}:${minute} ${hour24>=12?"PM":"AM"}`;}
  const date=new Date(value);if(Number.isNaN(date.getTime()))return "—";
  return new Intl.DateTimeFormat("en-US",{hour:"numeric",minute:"2-digit"}).format(date);
}
export function relativeUpdated(value?:string){
  if(!value)return "Not loaded";const ms=Date.now()-new Date(value).getTime();if(!Number.isFinite(ms))return "Updated";
  const minutes=Math.max(0,Math.round(ms/60000));return minutes<1?"Just now":`${minutes} min ago`;
}
...
export function useBaitLogicConditions(){
  ...
  const loadAt=useCallback(async(lat:number,lon:number,acc?:number)=>{
    setStatus(snapshotRef.current?"cached":"loading");setAccuracy(acc);
    try{
      const weatherResponse=await fetchWithTimeout(`/api/barometer-snapshot?lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lon)}`,9000);
      ...
      if(weatherSource==="offline-cache"){
        ...
        if(!Number.isFinite(age)||age<0||age>WEATHER_CACHE_MAX_AGE_MS)throw new Error("Saved weather conditions are too old to use safely.");
      }
      snapshotRef.current=weatherData;setSnapshot(weatherData);localStorage.setItem(CACHE_KEY,JSON.stringify(weatherData));
      setStatus(weatherSource==="offline-cache"?"cached":"live");

      fetchWithTimeout(`/api/water-snapshot?lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lon)}`,5000)
        .then(async response=>{
          if(!response.ok)throw new Error("water unavailable");
          const data=await response.json();const source=response.headers.get("X-BaitLogic-Source");
          const age=data?.timestamp?Date.now()-new Date(data.timestamp).getTime():Infinity;
          if(source==="offline-cache"&&(!Number.isFinite(age)||age<0||age>WATER_CACHE_MAX_AGE_MS))throw new Error("stale water");
          waterRef.current=data;setWater(data);setWaterStatus(source==="offline-cache"?"cached":"live");
        })
        .catch(()=>{waterRef.current=undefined;setWater(undefined);setWaterStatus("unavailable");});
    }catch{
      const fallback=readCache();
      if(fallback){snapshotRef.current=fallback;setSnapshot(fallback);setStatus("cached");}else setStatus("unavailable");
    }
  },[]);

  const refreshLocation=useCallback(()=>{
    setStatus(snapshotRef.current?"cached":"loading");
    if(!navigator.geolocation){loadAt(HIGHLAND.lat,HIGHLAND.lon);return;}
    navigator.geolocation.getCurrentPosition(
      p=>loadAt(p.coords.latitude,p.coords.longitude,p.coords.accuracy),
      ()=>loadAt(HIGHLAND.lat,HIGHLAND.lon),
      {enableHighAccuracy:false,timeout:5000,maximumAge:300000}
    );
  },[loadAt]);

  useEffect(()=>{
    const on=()=>setOnline(true),off=()=>setOnline(false);addEventListener("online",on);addEventListener("offline",off);
    return()=>{removeEventListener("online",on);removeEventListener("offline",off)};
  },[]);
  useEffect(()=>{refreshLocation();const timer=setInterval(()=>{if(navigator.onLine)refreshLocation();},60*60*1000);return()=>clearInterval(timer);},[refreshLocation]);

  const waterTemp=useMemo(()=>{const station=water?.stations?.find(s=>Number.isFinite(s.temp));return station?.temp??null;},[water]);
