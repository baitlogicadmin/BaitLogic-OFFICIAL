...
export function formatClock(value?: string|null) {
  ...
  if (localClock) {
    const hour24=Number(localClock[1]); const minute=localClock[2];
    if (Number.isFinite(hour24)) return `${hour24%12||12}:${minute} ${hour24>=12?"PM":"AM"}`;
  }
  ...
  const loadAt=useCallback(async(lat:number,lon:number,acc?:number)=>{
    ...
    try {
      ...
      fetchWithTimeout(`/api/water-snapshot?lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lon)}`,5000)
        .then(async waterResponse=>{
          if(!waterResponse.ok) throw new Error("water unavailable");
          ...
          if(waterSource==="offline-cache"&&(!Number.isFinite(waterAge)||waterAge<0||waterAge>WATER_CACHE_MAX_AGE_MS)) throw new Error("stale water");
          waterRef.current=waterData; setWater(waterData); setWaterStatus(waterSource==="offline-cache"?"cached":"live");
        })
        .catch(()=>{waterRef.current=undefined;setWater(undefined);setWaterStatus("unavailable");});
    } catch {
      const fallback=readCache();
      if(fallback){snapshotRef.current=fallback;setSnapshot(fallback);setStatus("cached");}
